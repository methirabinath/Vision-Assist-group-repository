...
class _NavigationScreenState extends State<NavigationScreen> {
  ...
  Future<void> _buildRoute(LatLng start, LatLng end) async {
    ...
    _markers.add(Marker(markerId: const MarkerId("start"), position: start, infoWindow: const InfoWindow(title: "You")));
    _markers.add(Marker(markerId: const MarkerId("end"), position: end, infoWindow: InfoWindow(title: widget.destinationText)));
    ...
    if (data["status"] != "OK") {
      throw Exception("Directions failed: ${data["status"]} (${data["error_message"] ?? ""})");
    }
    ...
    await _tts.speak("Route ready. $distanceText. Estimated time $durationText.");
  }
